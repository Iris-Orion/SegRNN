...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.rnn_type == "rnn":
            self.rnn = nn.RNN(input_size=self.num_layer, hidden_size=self.num_layer, num_layers=1, bias=True,
                              batch_first=True, bidirectional=False)
        elif self.rnn_type == "gru":
            self.rnn = nn.GRU(input_size=self.num_layer, hidden_size=self.num_layer, num_layers=1, bias=True,
                              batch_first=True, bidirectional=False)
    ...
    def forward(self, x):
        ...
        # b:batch_size c:channel_size s:H s:H
        # d:num_layer w:seg_len n:seg_num_x m:seg_num_y
        batch_size = x.size(0)
        ...
        if self.dec_way == "rmf":
            ...
            for i in range(self.seg_num_y):
                ...
                if self.rnn_type == "lstm":
                    ...
                    _, hn = self.rnn(yy, hn)
            y = torch.stack(y, dim=1).squeeze(2).reshape(-1, self.enc_in, self.L) # b,c,s
        elif self.dec_way == "pmf":
            if self.channel_id:
            ...
            if self.rnn_type == "lstm":
                ...
                _, hy = self.rnn(pos_emb, hn.repeat(1, 1, self.seg_num_y).view(1, -1, self.num_layer))
